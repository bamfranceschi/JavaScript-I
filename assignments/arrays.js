...
let carModels = inventory.map(e => e.car_model);
...
let carYears = [];
  for (e = 0; e < inventory.length; e++) {
    carYears.push(inventory[e].car_year); 
  }
...
// let oldCars = [carYears.filter(e => e.car_year < 2000)];
...
let oldCars = [];
  for (let e = 0; e < carYears.length; e++) {
  if (carYears[e] < 2000) { 
    oldCars.push(carYears[e]); 
  }
}
...
// A buyer is interested in seeing only BMW and Audi cars within the inventory. Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
 ...
 let BMWAndAudi = [];
  ...
  if (inventory[e].car_make === "BMW" || inventory[e].car_make === "Audi") { 
    BMWAndAudi.push(inventory[e].car_make);
    }
   }
 console.log(BMWAndAudi);

 //   inventory.filter(function(car_make){
//     return inventory.car_make === "BMW" || "Audi";
// })];